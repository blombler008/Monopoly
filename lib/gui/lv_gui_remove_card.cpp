...
void lv_create_remove_card_gui_async(void* user_card_ptr) {
    ...
    lv_obj_t * label_uid = lv_obj_assert_null(lv_label_create(src));
    lv_label_set_text(label_uid, ("UID: " + std::string(user_card->getUID())).c_str());
    lv_obj_align(label_uid, LV_ALIGN_LEFT_MID, 50, -20);

    lv_obj_t * label_card_number = lv_obj_assert_null(lv_label_create(src));
    ...
    lv_obj_t * label_name = lv_obj_assert_null(lv_label_create(src));
    lv_label_set_text(label_name, ("Name: " + std::string(user_card->getName())).c_str());
    lv_obj_align(label_name, LV_ALIGN_LEFT_MID, 50, 20);

    lv_obj_t * btn_confirm = lv_obj_assert_null(lv_button_create(src));
    lv_obj_align(btn_confirm, LV_ALIGN_BOTTOM_LEFT, 40, -30);
    lv_obj_t * label_confirm = lv_obj_assert_null(lv_label_create(btn_confirm));
    ...
    lv_obj_t * label_cancel = lv_obj_assert_null(lv_label_create(btn_cancel));
