...
void lv_create_remove_card_gui_async(void* user_card_ptr) {
    ...
    lv_create_title_underline(src, "Möchtest du die Karte\nwirklich entfernen?");
    ...
    lv_obj_t * label_uid = lv_obj_create_assert_null(lv_label_create(src));
    lv_label_set_text(label_uid, ("UID: " + std::string(user_card->getUID())).c_str());
    lv_obj_align(label_uid, LV_ALIGN_LEFT_MID, 50, -20);

    lv_obj_t * label_card_number = lv_obj_create_assert_null(lv_label_create(src));
    ...
    lv_obj_t * label_name = lv_obj_create_assert_null(lv_label_create(src));
    lv_label_set_text(label_name, ("Name: " + std::string(user_card->getName())).c_str());
    lv_obj_align(label_name, LV_ALIGN_LEFT_MID, 50, 20);

    lv_obj_t * btn_confirm = lv_obj_create_assert_null(lv_button_create(src));
    lv_obj_align(btn_confirm, LV_ALIGN_BOTTOM_LEFT, 40, -30);
    lv_obj_t * label_confirm = lv_obj_create_assert_null(lv_label_create(btn_confirm));
    lv_label_set_text(label_confirm, "Bestätigen");
    lv_obj_add_event_cb(btn_confirm, on_confirm_click, LV_EVENT_CLICKED, NULL);

    lv_obj_t * btn_cancel = lv_obj_create_assert_null(lv_button_create(src));
    lv_obj_align(btn_cancel, LV_ALIGN_BOTTOM_RIGHT, -40, -30);
    lv_obj_t * label_cancel = lv_obj_create_assert_null(lv_label_create(btn_cancel));
...
void lv_create_remove_card_gui(Card* user_card) {
    time_gui_creation_async(lv_create_remove_card_gui_async, user_card);
}
